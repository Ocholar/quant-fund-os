...
# ============================================================
# QFOS OUTLIER LOSS CAP â€” percentage based, equity-scaled
# Purpose:
...
def _qfos_expectancy_guard_with_cycle_log_inner(proposed_fills=None, context=None):
    ...
    try:
        ...
        # QFOS FALLBACK SCOUT QUALITY GUARD â€” inline pre-expectancy filter
        try:
...
# Purpose: prevent duplicate/oversized SELL rows from Profit Engine,
...
def _qfos_atomic_upsert_position(conn, payload):
    ...
    conn.execute("""
        ...
        ON CONFLICT(symbol) DO UPDATE SET
            ...
            updated_at = DATETIME('now', '+3 hours')
    """, (
        payload['symbol'], payload['quantity'], payload['avg_entry'], payload['realized_pnl'],
        payload['unrealized_pnl'], payload['last_price'], payload['exposure'], payload['strategy'],
    ))
...
def _qfos_atomic_insert_trade(conn, payload):
    ...
    """, (
        payload['symbol'], payload['side'], payload['quantity'], payload['expected_price'],
        payload['fill_price'], payload['slippage_bps'], payload['pnl'], payload['strategy'],
        payload['confidence'], int(bool(payload['live'])), int(bool(payload['shadow_mode'])), created_at,
    ))
...
def log_cycle_diagnostic(market_data=None, features=None, orders=None, portfolio=None, rejected=None, note=''):
    ...
    print('\n' + '=' * 72)
    print('QUANT FUND OS â€” CYCLE DIAGNOSTIC')
...
def main():
    ...
    while True:
        try:
            ...
            # POLICY V2 FIXED â€” persist normal feature map immediately after feature build
            try:
            ...
            if not ready:
                ...
                if fallback_features:
                    ...
                    # POLICY V2 â€” persist current feature map for trade classification
                    try:
            ...
            # QFOS ALLOCATOR OPPORTUNITY RESCUE â€” upstream rescue before fallback scout
            try:
            ...
            if not result.get('orders'):
                ...
                if scout_order:
                    ...
                    # POLICY V2 FIXED â€” source-block weak fallback immediately after injection
                    try:
                    ...
                    # QFOS FALLBACK SCOUT SOURCE GUARD â€” directly after scout injection
                    try:
            ...
            # POLICY V2 â€” block weak fallback scout before ORDERS/EXPECTANCY
            try:
            ...
            # POLICY V2 FIXED â€” block weak fallback orders in the real result['orders'] list
            try:
                if isinstance(result, dict) and isinstance(result.get('orders'), list):
                    result['orders'] = _qfos_v2_filter_fallback_orders(
                        result.get('orders', []),
...
# ============================================================
# QFOS SAFE WRAPPER PATCH â€” ACTIVE OUTLIER LOSS + BIG LOSER COOLDOWN
...
# ============================================================
# QFOS BASKET LOSS GUARD â€” ACTIVE WRAPPER
# Purpose:
...
# ============================================================
# QFOS EMERGENCY BASKET WATCHDOG â€” DB-LEVEL PAPER EXIT
# Purpose:
...
# ============================================================
# QFOS ACTIVE POSITION WATCHDOG â€” DB-LEVEL EXIT PROTECTION
# Purpose:
...
# ============================================================
# QFOS PROFIT ENGINE V1 â€” TRADE CLASS + PARTIAL TP + RUNNERS
# Purpose:
...
# ============================================================
# QFOS POLICY V2 â€” CONSOLIDATED ENTRY CLASSIFICATION + RUNNER LOGIC
# Purpose:
