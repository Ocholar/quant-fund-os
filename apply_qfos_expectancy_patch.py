...
ROOT = Path.cwd()
MAIN = ROOT / "main.py"
HELPER = ROOT / "qfos_expectancy_patch.py"
CONFIG = ROOT / "qfos_expectancy_config.json"

print(f"QFOS Defensive Expectancy Patch running in: {ROOT}")

if not MAIN.exists():
    raise FileNotFoundError("main.py not found. Run from C:\\Users\\Administrator\\Documents\\quant-fund-os")

stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
backup_dir = ROOT / f"backup_expectancy_patch_{stamp}"
...
config = {
    ...
    # Main 24h finding:
    # Bot is stable but losses are still larger than wins.
    # So reduce SIDEWAYS scout risk and improve exits.
    "sideways_scout_notional_usd": 1.00,
    "trend_scout_notional_usd": 1.50,

    # Block fake/micro breakouts.
    ...
    # Dead trade cleaner for SIDEWAYS.
...
if "from qfos_expectancy_patch import qfos_expectancy_cycle_guard" not in text:
    if "from qfos_winning_strategy import allow_risk_off_exit" in text:
        text = text.replace(
            "from qfos_winning_strategy import allow_risk_off_exit",
            "from qfos_winning_strategy import allow_risk_off_exit\nfrom qfos_expectancy_patch import qfos_expectancy_cycle_guard",
            1,
        )
    else:
        ...
        for i, line in enumerate(lines[:180]):
            if line.startswith("import ") or line.startswith("from "):
                insert_at = i + 1
        lines.insert(insert_at, "from qfos_expectancy_patch import qfos_expectancy_cycle_guard")
        text = "\n".join(lines) + "\n"

guard_block = """
try:
    proposed_fills = qfos_expectancy_cycle_guard(proposed_fills, locals())
except Exception as _qfos_expectancy_error:
    print(f"[EXPECTANCY_PATCH] guard failed: {_qfos_expectancy_error}")
"""

if "qfos_expectancy_cycle_guard(proposed_fills, locals())" not in text:
    patched = False

    patterns = [
        r'(?m)^([ \t]*)print\((["\'])ORDERS:\2,\s*proposed_fills\)',
        r'(?m)^([ \t]*)print\((["\'])ORDERS:\2,\s*orders\)',
    ]
    ...
    for pat in patterns:
        ...
        if m:
            indent = m.group(1)
            block = "\n".join(indent + line for line in guard_block.strip().splitlines())
            text = text[:m.start()] + block + "\n" + text[m.start():]
            patched = True
            break

    if not patched:
        pat = r'(?m)^([ \t]*)for\s+[^:\n]+\s+in\s+proposed_fills\s*:'
        m = re.search(pat, text)
        if m:
            indent = m.group(1)
            block = "\n".join(indent + line for line in guard_block.strip().splitlines())
    ...
    if not patched:
        raise RuntimeError(
            "Could not find a safe proposed_fills hook in main.py. "
            "Search main.py for ORDERS or proposed_fills and send that section."
        )
...
print("Touched files:")
print(" - main.py")
print(" - qfos_expectancy_patch.py")
print(" - qfos_expectancy_config.json")
print("")
print("Expected behavior:")
print(" - SIDEWAYS fallback scout trades shrink to about $1.00")
print(" - micro-price coins below $0.01 are blocked")
print(" - weak scout entries are blocked")
print(" - extreme-volatility scout traps are blocked")
print(" - repeated scout losers get cooldown")
print(" - breakeven_protection_exit can appear")
print(" - time_stop_exit can appear")
print(" - trailing_profit_exit remains active")
print(" - risk_off_exit logic remains untouched")
