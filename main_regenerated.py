# ============================================================
# QFO REGENERATED MAIN — 2026-06-03
# Source: uploaded main(2).py, syntax-checked and hardened.
# Key integrated fixes:
# - Rescue hourly cap default = 4 in SIDEWAYS.
# - SIDEWAYS hard exposure veto remains 4.5%.
# - Profit Engine stale class downgrade helper defined explicitly.
# - File generated as a full main.py replacement, not an anchor patch.
# ============================================================
...
def _qfos_guard_policy_class_from_features(symbol, result_f_by_symbol=None, features=None):
    """
    Profit Engine stale-class classifier bridge.

    Uses the latest available Policy V2 / feature snapshot to decide whether a
    previously trend-classified position should now be managed as SIDEWAYS_SCALP.
    This prevents old QUALITY_TREND_OR_BREAKOUT positions from receiving wide
    trend-style exit handling after the market has reverted to sideways scalp
    conditions.
    """
    try:
        feature_map = result_f_by_symbol if isinstance(result_f_by_symbol, dict) else features
        if not isinstance(feature_map, dict):
            feature_map = {}
        f = feature_map.get(symbol) or {}
        if not isinstance(f, dict):
            f = {}

        symbol_regime = str(f.get("symbol_regime") or "").upper()
        is_up = bool(f.get("is_symbol_uptrend", False))
        is_down = bool(f.get("is_symbol_downtrend", False))
        is_choppy = bool(f.get("is_choppy", False))
        trend_quality = _qfos_guard_float(f.get("trend_quality") or f.get("symbol_trend_score"))
        breakout = _qfos_guard_float(f.get("breakout_score"))
        momentum = _qfos_guard_float(f.get("momentum"))

        if symbol_regime in ("SYMBOL_TREND_UP", "SYMBOL_BREAKOUT_UP") and not is_down and not is_choppy:
            if trend_quality >= 0.003 or breakout >= 0.003 or momentum >= 0.0005 or is_up:
                return "QUALITY_TREND_OR_BREAKOUT"

        return "SIDEWAYS_SCALP"
    except Exception as exc:
        print(f"[PROFIT_ENGINE_GUARD] policy_class_feature_error symbol={symbol} err={exc}", flush=True)
        return None
