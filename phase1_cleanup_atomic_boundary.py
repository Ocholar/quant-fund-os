...
def qfos_persist_fill_atomic(conn, fill, source="main_loop"):
    """
    Single atomic paper fill persistence boundary.

    SELL invariants:
    - open_qty <= 0 rejects before trade insert
    - requested_qty <= 0 rejects before trade insert
    - requested_qty > open_qty caps to open_qty
    - repeated full-position sell after zero rejects
    - position update happens before trade insert
    - realized PnL uses DB avg_entry
...
# 1. Remove all marker-based copies.
text = re.sub(
...
# 2. Remove legacy Agent-1 atomic helper functions that caused bypass-scan hits.
# Do not remove unrelated qfos helpers like expectancy helpers.
legacy_names = {
...
try:
    ...
    raise SystemExit(f"Cannot parse main.py before cleanup: {e}")
...
# 3. Insert one canonical boundary before def main().
...
print("Removed legacy atomic helpers:", ", ".join(sorted({name for _, _, name in spans})) or "none")
print("Inserted single canonical qfos_persist_fill_atomic() boundary.")
